...
__global__ void compute_scalar_field_point_kernel(float* point_values,
		float* field_data,
		int* batch_offsets,
		int* num_points,
		float* radius,
	...
	int shared_data_size = 3 * *num_points;
	...
	float sum = 0.0f;
	for (int i = 0; i < shared_data_size; i += 3) {
		...
		if (rsq < maxsq) {
			sum += evaluate_kernel(rsq, &coefficients);
		}
	}
...
void launch_scalar_field_point_kernel(dim3 blockSize, dim3 gridSize,
	...
	compute_scalar_field_point_kernel<<<gridSize, blockSize, shared_mem_size>>>(pointvalues_d, vfield_d, offsets_d, num_points_d, radius_d, cellsize_d);
